...
v0.1.3 以视频节点第一批能力和画布可用性收口为主：

- Seedance 视频节点支持首帧 / 尾帧、普通图片全能参考与自适应参数；后续生成失败时保留旧结果，并可翻页查看视频历史。
- Seedance Mini 的普通图片全能参考 + 自适应已完成一次真实 Provider 生成回流验证。
- 新增可选 Seedance Mini、Kling V3 和 Kling 3.0 Turbo 预设。Kling 已完成预设、参数能力和主动添加入口验收，但尚未完成真实 Kling Provider 生成验证。
- 新增应用配色、项目级画布底纹和 4× 缩放；拼接图像可以作为普通图片输入接入 ComfyUI。

## 发布包内容

- 32 条模型连接模板，所有 Key 均为空。
...
3. 双击 `ZZHB\ZZHB.exe`，等待本地服务启动并自动打开浏览器。
